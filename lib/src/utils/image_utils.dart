...
class ImageUtils {
  ...
  Future<Area?> findContourPhoto(
    ...
    try {
      final contour = await DocumentScanner.findContourPhoto(
        byteData: byteData,
  ...
  Future<Uint8List?> adjustingPerspective(
    ...
    try {
      final newImage = await DocumentScanner.adjustingPerspective(
        byteData: byteData,
        contour: contour,
      );
  ...
  Future<Uint8List> applyFilter(
    ...
    try {
      final newImage = await DocumentScanner.applyFilter(
        byteData: byteData,
        filter: filter,
      );
